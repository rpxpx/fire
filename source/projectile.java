...
public void draw() {
  ...
  l.setS(0.2f * (sqrt(pow(mouseX-oX, 2) + pow(height-mouseY-l.getY(), 2))));
  l.draw();
...
class TargetSet{
  ...
  TargetSet(int n, int s){
    ...
    ei = 0;
    for( int i=0; i<tn; i++ ){
      t[i] = new Target(10+PApplet.parseInt(random(width-20)), 10+PApplet.parseInt(random(height-20)), PApplet.parseInt(random(s))+s/2 ); 
    }
  }
...
class Explosion{
  ...
  Explosion(int xpos, int ypos, int size, float vel){
      ...
      for( int i=0; i<n; i++ ){
        p[i] = new Projectile(random(1)*TWO_PI, (random(1)+0.5f)*v, x, y, (size/18.0f) );
        //p[i] = new Projectile(random(1)*TWO_PI, random(2)*v, x, y, 2 );
      }
  }
  
  public Boolean advance(float t){
  ...
  public void draw(){
    for( int i=0; i<n; i++ ){
      ...
      stroke(255, (p[i].getV()/100.0f)*255, (v/90)*255);
      p[i].draw();
    }     
  }
}
...
class Launcher {
  ...
  public void draw() {
    stroke(fg);
    strokeWeight(2);
    line(x, height-y, x+(2.0f*s*cos(a)), height-y-(2.0f*s*sin(a)));  
  }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "projectile" };
    if (passedArgs != null) {
